...
async def whisper_install():
    ...
    OBS: este runtime foi migrado de mlx-whisper (só Apple Silicon) para
    faster-whisper (CTranslate2, Windows/Linux/macOS) — ver whisper_runtime.py.
    Um bloqueio antigo aqui ("só funciona em macOS") ficou esquecido depois
    dessa migração e impedia a instalação em qualquer máquina Windows/Linux;
    removido porque o pacote atual (WHISPER_PACKAGES) não tem essa restrição.
    """
    from backend.services import whisper_runtime
